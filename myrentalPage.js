...
xhr.open('GET', 'api/myrentals.php?action=list');
    ...
    if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if (res.code == 0) {
            ...
            for (var i = 0; i < res.data.length; i++) {
                ...
                str += '<li style="margin-right:40px">\n<form action="./api/myrentals.php" name="removeRental" method="post">' +
                    '<img src="api/upload/' + res.data[i].bedroom_img + '"  style="height: 250px; width: 300px">' +
                    '<span class="text-light d-flex mx-auto justify-content-center">' + res.data[i].property_type +
                    '&nbsp;<span className="a8jt5op dir dir-ltr" ></span>' +
                    '<span aria-hidden="true"></span>' + res.data[i].bedroom_num +
                    ' bed ' + res.data[i].bathroom_num + ' bath</span>' +
                    '<p class="text-light d-flex mx-auto justify-content-center">' +
                    res.data[i].address + '</p>' +
                    '<input type="submit" class="btn btn-danger d-flex mx-auto justify-content-center" name="remove" value="Remove for $' + res.data[i].rent + '">' +
